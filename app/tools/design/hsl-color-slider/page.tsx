...
import { hslColorSliderConfig as config } from "@/tools/hsl-color-slider/config";

const HSLColorSliderUI = dynamic(() => import("@/tools/hsl-color-slider/ui"));

const canonicalUrl = `${siteConfig.url}/tools/design/hsl-color-slider`;

const seo = (config as any).seo ?? {};
const toolName = (config as any).name ?? (config as any).title ?? "hsl-color-slider";
...
export default function HSLColorSliderPage() {
  const catObj = categories.find((c) => c.slug === "design");
