import { MomentumInputs, MomentumResult, HistoryEntry, MassUnit, VelocityUnit } from "./types";
...
export const ALL_MASS_UNITS: MassUnit[]    = ["kg", "g", "mg", "lb"];
...
export function calculate(inputs: MomentumInputs): MomentumResult | null {
  ...
  const p      = massKg * velMs;

  const formula = `p = ${formatNum(massKg, 4)} kg × ${formatNum(velMs, 4)} m/s = ${formatNum(p, 4)} kg·m/s`;

  return {
    momentumKgMs:  p,
    momentumGMs:   p * 1000,
    momentumLbFts: p * 0.224809 / 0.3048,   // kg·m/s → lb·ft/s
    massKg,
    velMs,
    formula,
  };
...
const HISTORY_KEY = "momentum-calculator-history";
const MAX_HISTORY = 10;

export function saveToHistory(inputs: MomentumInputs, result: MomentumResult): void {
  try {
...
export function exportToText(inputs: MomentumInputs, result: MomentumResult): string {
  const p = inputs.precision;
  return [
    "Momentum Calculator – Result",
    "=".repeat(45),
    "",
    ...
    "",
    `Momentum (kg·m/s)  : ${formatNum(result.momentumKgMs, p)}`,
    `Momentum (g·m/s)   : ${formatNum(result.momentumGMs, p)}`,
    `Momentum (lb·ft/s) : ${formatNum(result.momentumLbFts, p)}`,
    "",
