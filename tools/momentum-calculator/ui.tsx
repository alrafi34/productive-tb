...
import { MomentumInputs, MomentumResult, HistoryEntry, MassUnit, VelocityUnit, Precision } from "./types";
import {
  calculate,
  validateMass,
  validateVelocity,
...
const PRESETS = [
  ...
  { label: "Sprinter",      mass: "80",    massUnit: "kg" as MassUnit, velocity: "10",   velocityUnit: "m/s"  as VelocityUnit },
  { label: "Freight Train", mass: "50",    massUnit: "kg" as MassUnit, velocity: "60",   velocityUnit: "mph"  as VelocityUnit },
];
...
export default function MomentumCalculatorUI() {
  const [inputs,      setInputs]      = useState<MomentumInputs>(DEFAULT_INPUTS);
  ...
  const handleExport = () => {
    if (!result) return;
    downloadFile(exportToText(inputs, result), "momentum-calculation.txt");
  };
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-blue-900 mb-1">Momentum Calculator (p = m × v)</h3>
              <p className="text-sm text-blue-800">
                Enter mass and velocity to instantly calculate the linear momentum of an object. Supports metric and imperial units. Negative velocity produces negative momentum.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-primary rounded-xl border border-primary-light shadow-lg shadow-primary/20 p-6 text-white">
              ...
              <div className="text-3xl font-bold mb-1 leading-none break-all">
                {result !== null ? `${formatNum(result.momentumKgMs, inputs.precision)} kg·m/s` : "—"}
              </div>

              {result && (
                <div className="pt-4 border-t border-white/20 text-sm space-y-1.5 mb-4 mt-3">
                  <div className="flex justify-between">
                    <span className="text-primary-100">kg·m/s</span>
                    <span className="font-semibold">{formatNum(result.momentumKgMs, inputs.precision)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-primary-100">g·m/s</span>
                    <span className="font-semibold">{formatNum(result.momentumGMs, inputs.precision)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-primary-100">lb·ft/s</span>
                    <span className="font-semibold">{formatNum(result.momentumLbFts, inputs.precision)}</span>
                  </div>
                </div>
              )}
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Mass
                    <span className="ml-1 text-xs text-gray-400" title="Must be a positive number greater than zero">ⓘ</span>
                  </label>
                  <input
                    ref={massRef}
                    type="number"
                  ...
                  <p className="text-xs text-gray-400 mt-1">e.g. Car ≈ 1,500 kg</p>
                </div>
                <div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Velocity
                    <span className="ml-1 text-xs text-gray-400" title="Negative velocity produces negative momentum">ⓘ</span>
                  </label>
                  <input
                    type="number"
                    inputMode="decimal"
                  ...
                  <p className="text-xs text-gray-400 mt-1">Negative value = opposite direction</p>
                </div>
                <div>
              ...
              {result && (
                <div className="p-3 bg-green-50 border border-green-200 rounded-lg text-sm text-green-800">
                  ...
                  p = m × v = {inputs.mass} {MASS_SHORT[inputs.massUnit]} × {inputs.velocity} {VEL_SHORT[inputs.velocityUnit]} = <strong>{formatNum(result.momentumKgMs, inputs.precision)} kg·m/s</strong>
                </div>
              )}

              {result && (
                <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-700">
                  An object with a mass of <strong>{formatNum(result.massKg, 2)} kg</strong> moving at <strong>{formatNum(result.velMs, 2)} m/s</strong> has a momentum of <strong>{formatNum(result.momentumKgMs, inputs.precision)} kg·m/s</strong>.
                </div>
              )}
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      <tr className="bg-primary/5">
                        <td className="py-2 px-3 font-medium text-primary">kg·m/s</td>
                        <td className="py-2 px-3 font-mono font-semibold text-primary">{formatNum(result.momentumKgMs, inputs.precision)}</td>
                        <td className="py-2 px-3 text-gray-500">SI standard unit</td>
                      </tr>
                      <tr>
                        <td className="py-2 px-3 font-medium text-gray-700">g·m/s</td>
                        <td className="py-2 px-3 font-mono">{formatNum(result.momentumGMs, inputs.precision)}</td>
                        <td className="py-2 px-3 text-gray-500">Gram-meter per second</td>
                      </tr>
                      <tr>
                        <td className="py-2 px-3 font-medium text-gray-700">lb·ft/s</td>
                        <td className="py-2 px-3 font-mono">{formatNum(result.momentumLbFts, inputs.precision)}</td>
                        <td className="py-2 px-3 text-gray-500">Pound-foot per second (US)</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
                ...
                <div className="space-y-3 text-sm text-gray-700">
                  ...
                  <div className="space-y-2">
                    {[
                      `Step 1: p = m × v`,
                      `Step 2: Convert mass → ${formatNum(result.massKg, 4)} kg`,
                      `Step 3: Convert velocity → ${formatNum(result.velMs, 4)} m/s`,
                      `Step 4: p = ${formatNum(result.massKg, 4)} × ${formatNum(result.velMs, 4)}`,
                      `Step 5: p = ${formatNum(result.momentumKgMs, inputs.precision)} kg·m/s`,
                    ].map((step, i) => (
                  ...
                  <div className="grid sm:grid-cols-3 gap-3 pt-2">
                    <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-center">
                      <div className="font-semibold text-blue-800 text-xs uppercase mb-1">p (Momentum)</div>
                      <div className="text-blue-700 text-xs">Measured in kg·m/s. Quantity of motion.</div>
                    </div>
                    ...
                    <div className="p-3 bg-green-50 border border-green-200 rounded-lg text-center">
                      ...
                      <div className="text-green-700 text-xs">Measured in m/s. Speed with direction.</div>
                    </div>
                  </div>
                  <div className="p-3 bg-purple-50 border border-purple-200 rounded-lg text-sm text-purple-800">
                    <strong>Direction matters:</strong> Momentum is a vector quantity. A negative velocity means the object moves in the opposite direction, producing negative momentum. This is key in collision analysis.
                  </div>
                </div>
              </div>
            )}
                    ...
                    history.map((entry) => (
                      ...
                      >
                        ...
                        <div className="text-sm font-semibold text-primary">
                          = {formatNum(entry.result.momentumKgMs, 2)} kg·m/s
                        </div>
                      </div>
                    ))
                  )}
