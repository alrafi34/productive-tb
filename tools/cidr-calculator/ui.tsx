...
const PRESETS = [
  ...
  { label: "172.16.50.10/28", ip: "172.16.50.10", cidr: 28 },
  { label: "192.168.1.1/16", ip: "192.168.1.1", cidr: 16 },
  { label: "10.10.0.1/30", ip: "10.10.0.1", cidr: 30 },
];
...
export default function CidrCalculatorUI() {
  ...
  const [inputMode, setInputMode] = useState<"cidr" | "manual">("cidr");
  ...
  const run = useCallback(
    debounce((ipVal: string, cidrVal: number) => {
      if (!isValidIp(ipVal)) {
        setInputError("Please enter a valid IPv4 address (e.g. 192.168.1.1)");
        setResult(null);
        return;
      }
      setInputError(null);
  ...
  const handleCidrInput = (val: string) => {
    ...
    if (parsed) {
      ...
      setInputError("Format: 192.168.1.1/24 (CIDR must be 0–32)");
    }
  };
  ...
  const handleMaskInput = (val: string) => {
    ...
    if (parsed !== null) {
      setCidr(parsed);
      setCidrInput(`${ip}/${parsed}`);
    }
  };

  const handleCidrSlider = (val: number) => {
    setCidr(val);
    setCidrInput(`${ip}/${val}`);
  };

  const handleIpManual = (val: string) => {
    const cleaned = val.replace(/[^0-9.]/g, "");
    setIp(cleaned);
    setCidrInput(`${cleaned}/${cidr}`);
  };

  const handlePreset = (p: (typeof PRESETS)[0]) => {
    setIp(p.ip);
    setCidr(p.cidr);
    setCidrInput(`${p.ip}/${p.cidr}`);
    if (inputMode === "cidr") cidrRef.current?.focus();
    else ipRef.current?.focus();
  };

  const handleReset = () => {
    setIp("192.168.1.1");
    setCidr(24);
    setCidrInput("192.168.1.1/24");
    setResult(null);
    setInputError(null);
    if (inputMode === "cidr") cidrRef.current?.focus();
    else ipRef.current?.focus();
  ...
  const handleExportJson = () => {
    if (!result) return;
    const blob = new Blob(
      [JSON.stringify({ ip: result.ip, cidr: `/${result.cidr}`, subnetMask: result.subnetMask, wildcardMask: result.wildcardMask, networkAddress: result.networkAddress, broadcastAddress: result.broadcastAddress, firstHost: result.firstHost, lastHost: result.lastHost, usableRange: `${result.firstHost} – ${result.lastHost}`, totalHosts: result.totalHosts, usableHosts: result.usableHosts, ipClass: result.ipClass, networkType: result.ipType }, null, 2)],
      { type: "application/json" }
    );
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `cidr-calculation.json`;
    a.click();
  ...
  const handleExportTxt = () => {
    ...
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `cidr-calculation.txt`;
    a.click();
    URL.revokeObjectURL(a.href);
  };
  ...
  const RESULT_ROWS = result
    ? [
        ...
        { label: "CIDR Prefix", value: `/${result.cidr}`, mono: true },
          ...
          <div className="lg:col-span-4 space-y-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => setInputMode(inputMode === "cidr" ? "manual" : "cidr")}
                  className="text-xs text-primary font-medium hover:underline"
                >
                  {inputMode === "cidr" ? "Switch to Manual" : "Switch to CIDR"}
                </button>
              </div>
              ...
              {inputMode === "cidr" ? (
                <div>
                  ...
                  <input
                    ...
                    aria-label="IP Address with CIDR"
                  />
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      type="text"
                      value={ip}
                      onChange={(e) => handleIpManual(e.target.value)}
                      onKeyDown={(e) => e.key === "Escape" && handleReset()}
                      className={`w-full px-3 py-2.5 border-2 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent font-mono text-sm ${
                        inputError ? "border-red-300" : "border-gray-200"
                      }`}
              ...
              <div>
                ...
                <input
                  ...
                  max={32}
                  value={cidr}
                  onChange={(e) => handleCidrSlider(parseInt(e.target.value))}
              ...
              {/* Quick CIDR presets */}
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1.5">Quick Select</label>
                <div className="flex flex-wrap gap-1.5">
                  {[8, 16, 24, 28, 30, 31, 32].map((c) => (
                    <button
                      key={c}
                      onClick={() => handleCidrSlider(c)}
            ...
            <div className="bg-primary rounded-xl border border-primary shadow-lg shadow-primary/20 p-5 text-white">
              ...
              {result ? (
                ...
                <div className="text-primary-100 text-sm">Enter an IP/CIDR to calculate</div>
              )}
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              {result ? (
                ...
                <div className="p-8 text-center text-gray-400 text-sm">
                  {inputError ? inputError : "Enter an IP/CIDR address to see results"}
                </div>
              )}
            </div>
                    ...
                    history.map((entry) => (
                      <div
                        ...
                        onClick={() => {
                          setIp(entry.ip);
                          setCidr(entry.cidr);
                          setCidrInput(`${entry.ip}/${entry.cidr}`);
                      ...
                      >
                        ...
                        <div className="text-xs text-primary font-mono">
                          Network: {entry.result.networkAddress} · Hosts: {formatNumber(entry.result.usableHosts)}
                        </div>
                      </div>
                    ))
                  )}
