...
const PRESETS: { label: string; value: string; unit: Unit }[] = [
  ...
  { label: "5 Decimal",  value: "5",   unit: "decimal" },
  { label: "10 Decimal", value: "10",  unit: "decimal" },
  { label: "1 Acre",     value: "1",   unit: "acre"    },
  { label: "1 Katha",    value: "1",   unit: "katha"   },
];

export default function DecimalLandCalculatorUI() {
  ...
  const handleCopy = () => {
    ...
    const lines = ALL_UNITS
      ...
      .join("\n");
    navigator.clipboard.writeText(`${formatNumber(val, inputs.precision)} ${fromLabel} (${region})\n${lines}`);
    setCopied(true);
  ...
  const handleExport = () => {
    if (!result) return;
    downloadFile(exportToText(inputs, result), "decimal_land_calculation.txt");
  };
  ...
  // Primary result: if input is decimal show sqft, otherwise show decimal
  const primaryUnit: Unit = inputs.fromUnit === "decimal" ? "sqft" : "decimal";
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-blue-900 mb-1">Decimal Land Calculator</h3>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
              ...
              <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-600 space-y-1">
                ...
                <div>1 Decimal = {table.decimal.toLocaleString()} sq ft</div>
                <div>1 Katha = {table.katha.toLocaleString()} sq ft</div>
                <div>1 Bigha = {table.bigha.toLocaleString()} sq ft</div>
                <div>1 Acre = 100 Decimal</div>
              </div>
            ...
            {result && (
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Same input ({formatNumber(parseFloat(inputs.value), inputs.precision)} {UNIT_LABELS[inputs.fromUnit]}) in Decimal across all regions:
                </p>
                <div className="space-y-2">
                  {(["bangladesh", "westbengal", "bihar", "nepal"] as Region[]).map((r) => {
                    const t = REGION_TABLES[r];
                    const sqft = parseFloat(inputs.value) * t[inputs.fromUnit];
                    const decimal = sqft / t.decimal;
                    return (
                      <div key={r} className="flex items-center justify-between p-3 bg-gray-50 border border-gray-200 rounded-lg">
                        ...
                        <div className="text-right">
                          <span className="font-mono font-semibold text-gray-900 text-sm">
                            {formatNumber(decimal, inputs.precision)} Decimal
                          </span>
                    ...
                    history.map((entry) => (
                      ...
                      >
                        ...
                        <div className="text-sm text-gray-600">
                          = {formatNumber(entry.result.decimal, entry.inputs.precision)} Decimal
