...
const PRESETS = [
  { label: "¼ Acre", value: "10890" },
  { label: "½ Acre", value: "21780" },
  { label: "1 Acre", value: "43560" },
  { label: "2 Acres", value: "87120" },
];

export default function SquareFeetToAcreConverterUI() {
  ...
  // Reverse mode: acres → sqft (simple inline calc, no history)
  const reverseResult =
    reversed && reverseValue !== ""
      ? parseFloat(reverseValue) * SQFT_PER_ACRE
      : null;
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard.writeText(
      `${formatNumber(result.squareFeet, 0)} sq ft = ${formatNumber(result.acres, inputs.precision)} acres`
    );
    setCopied(true);
  ...
  const handleExport = () => {
    if (!result) return;
    downloadFile(exportToText(inputs, result), "sqft_to_acre_conversion.txt");
  };
  ...
  return (
    <>
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-blue-900 mb-1">Square Feet to Acre Converter</h3>
              <p className="text-sm text-blue-800">
                Enter any square feet value to instantly convert it to acres. 43,560 sq ft = 1 acre.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
              ...
              <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
                ...
                <div className="text-sm font-mono text-gray-900">acres = sq ft ÷ 43,560</div>
              </div>

              <div className="pt-2 space-y-2">
                <button
                  onClick={() => { setReversed((r) => !r); setReverseValue(""); }}
                ...
                >
                  ⇄ {reversed ? "Back to sq ft → ac" : "Reverse: ac → sq ft"}
                </button>
                <button
                  onClick={handleReset}
            ...
            <div className="bg-primary rounded-xl border border-primary-light shadow-lg shadow-primary/20 p-6 text-white">
              ...
              >
                {reversed ? "Square Feet" : "Acres"}
              </p>
              <div className="text-4xl font-bold mb-1 leading-none break-all">
                {reversed
                  ? reverseResult !== null && !isNaN(reverseResult)
                    ? formatNumber(reverseResult, 0)
                    : "—"
                  : result
              ...
              {!reversed && result && (
                <div className="pt-4 border-t border-white/20 text-sm space-y-1 mb-4">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{formatNumber(result.squareFeet, 0)} sq ft</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-primary-100">Divisor:</span>
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
              <h3 className="font-semibold text-gray-800" style={{ fontFamily: "var(--font-heading)" }}>
                {reversed ? "Enter Acres" : "Enter Square Feet"}
              </h3>
              ...
              {reversed ? (
                <div>
                  ...
                  <input
                    ...
                    step="any"
                  />
                  {reverseResult !== null && !isNaN(reverseResult) && reverseValue !== "" && (
                    <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                      <div className="text-sm text-green-800">
                        <strong>Calculation:</strong> {reverseValue} × 43,560 ={" "}
                        <strong>{formatNumber(reverseResult, 0)} sq ft</strong>
                      </div>
                    </div>
                  )}
                </div>
                  ...
                  {result && !error && (
                    <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                      <div className="text-sm text-green-800">
                        ...
                        {formatNumber(result.squareFeet, 0)} ÷ 43,560 ={" "}
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {[10890, 21780, 43560, 87120, 217800, 435600, 1000000].map((sqft) => (
                      <tr
                        key={sqft}
                        className={`hover:bg-gray-50 cursor-pointer transition-colors ${
                          parseFloat(inputs.value) === sqft ? "bg-primary/5" : ""
                        }`}
                        onClick={() => handlePreset(String(sqft))}
                      >
                        <td className="py-2 px-3 font-mono text-gray-700">
                          {sqft.toLocaleString()}
                        </td>
                        <td className="py-2 px-3 font-mono font-semibold text-gray-900">
                          {formatNumber(sqft / SQFT_PER_ACRE, inputs.precision)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-semibold text-gray-900 text-sm">
                            {formatNumber(entry.result.squareFeet, 0)} sq ft
                          </span>
      ...
      <RelatedTools
        currentTool="square-feet-to-acre-converter"
        tools={[
