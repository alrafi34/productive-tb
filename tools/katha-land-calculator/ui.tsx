...
const PRESETS: { label: string; value: string; unit: Unit }[] = [
  { label: "1 Katha",   value: "1",   unit: "katha" },
  { label: "5 Katha",   value: "5",   unit: "katha" },
  { label: "10 Katha",  value: "10",  unit: "katha" },
  { label: "1 Decimal", value: "1",   unit: "decimal" },
  { label: "1 Bigha",   value: "1",   unit: "bigha" },
];

const OUTPUT_UNITS: Unit[] = ["katha", "decimal", "bigha", "acre", "sqft", "sqm", "hectare"];

export default function KathaLandCalculatorUI() {
  ...
  const handleReset = () => {
    setInputs({ value: "", fromUnit: "katha", region: "bangladesh", precision: 4 });
    setResult(null);
    setError(null);
    inputRef.current?.focus();
  };
  ...
  const handleSwap = () => {
    // no-op for now — swap is future-ready
  };
  ...
  const handleCopy = () => {
    ...
    const lines = OUTPUT_UNITS
      .filter((u) => u !== inputs.fromUnit)
  ...
  const handleExport = () => {
    if (!result) return;
    downloadFile(exportToText(inputs, result), "katha_land_calculation.txt");
  };
  ...
  // Primary result unit (same as fromUnit, or katha if fromUnit is katha)
  const primaryUnit: Unit = inputs.fromUnit === "katha" ? "decimal" : "katha";
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-blue-900 mb-1">Katha Land Calculator</h3>
              <p className="text-sm text-blue-800">
                Convert land area between Katha, Decimal, Bigha, Acre, and more using Bangladesh, West Bengal, Bihar, or Nepal standards.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
              ...
              <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-600 space-y-1">
                ...
                <div>1 Katha = {REGION_TABLES[inputs.region].katha.toLocaleString()} sq ft</div>
                <div>1 Bigha = {REGION_TABLES[inputs.region].bigha.toLocaleString()} sq ft</div>
            ...
            {result && (
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Same input ({formatNumber(parseFloat(inputs.value), inputs.precision)} {UNIT_LABELS[inputs.fromUnit]}) across all regions:
                </p>
                <div className="space-y-2">
                  {REGIONS.map((r) => {
                    const table = REGION_TABLES[r];
                    const sqft = parseFloat(inputs.value) * table[inputs.fromUnit];
                    const katha = sqft / table.katha;
                    return (
                      <div key={r} className="flex items-center justify-between p-3 bg-gray-50 border border-gray-200 rounded-lg">
                        ...
                        <div className="text-right">
                          <span className="font-mono font-semibold text-gray-900 text-sm">
                            {formatNumber(katha, inputs.precision)} Katha
                          </span>
                    ...
                    history.map((entry) => (
                      ...
                      >
                        ...
                        <div className="text-sm text-gray-600">
                          = {formatNumber(entry.result.katha, entry.inputs.precision)} Katha
