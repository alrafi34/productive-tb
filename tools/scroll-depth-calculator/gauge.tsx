...
export default function ScrollDepthGauge({ pct, color, size = 160 }: Props) {
  ...
  useEffect(() => {
    ...
    const valueSweep = (clamped / 100) * SWEEP;
    ctx.beginPath();
    ...
    ctx.fillText("scroll depth", cx, cy + 20);
  }, [pct, color, size]);

  return <canvas ref={canvasRef} role="img" aria-label={`Scroll depth ${pct.toFixed(0)} percent`} />;
}
