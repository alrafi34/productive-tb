...
export default function CapacitorChargeTimeCalculatorUI() {
  ...
  const [targetPercentage, setTargetPercentage] = useState<ChargePercentage>(99);
  ...
  const debouncedCalculate = useCallback(
    ...
    [resistance, resistanceUnit, capacitance, capacitanceUnit, targetPercentage, voltage]
  );
  ...
  const calculate = () => {
    ...
    try {
      const calcResult = calculateChargeTime(input);
  ...
  const handleLoadPreset = (preset: typeof COMMON_PRESETS[0]) => {
    ...
    setTargetPercentage(preset.targetPercentage);
  };

  const handleCopy = () => {
    if (result) {
      const chargeTimeFormatted = formatTime(result.chargeTime);
      const text = `${formatNumber(chargeTimeFormatted.value, 4)} ${chargeTimeFormatted.unit}`;
  ...
  const handleExportText = () => {
    if (result) {
      ...
      downloadFile(text, 'capacitor_charge_time_calculation.txt');
    }
  };
  ...
  const loadFromHistory = (entry: any) => {
    ...
    setTargetPercentage(entry.input.targetPercentage);
  ...
  const chargePercentages: ChargePercentage[] = [50, 63, 90, 95, 99];
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-blue-900 mb-1">Capacitor Charge Time Calculator</h3>
              <p className="text-sm text-blue-800">
                Calculate how long a capacitor takes to charge in an RC circuit. Get time constant (τ), charge time, and full charge time with instant results.
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="bg-primary rounded-xl border border-primary-light shadow-lg shadow-primary/20 p-6 text-white space-y-4">
                <div>
                  <p className="text-primary-100 font-medium mb-2 text-xs uppercase tracking-wider" style={{ fontFamily: "var(--font-heading)" }}>
                    Charge Time to {result.targetPercentage}%
                  </p>
                  <div className="text-4xl font-bold mb-1">
                    {formatNumber(formatTime(result.chargeTime).value, 4)}
                  </div>
                  <div className="text-xl text-primary-100">
                    {formatTime(result.chargeTime).unit}
                  </div>
                </div>
          ...
          <div className="lg:col-span-8 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {chargePercentages.map((percentage) => (
                      <button
                        key={percentage}
                        onClick={() => setTargetPercentage(percentage)}
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    {targetPercentage === 63 && "1τ - One time constant"}
                    {targetPercentage === 95 && "~3τ - Often considered 'charged'"}
                    {targetPercentage === 99 && "~5τ - Fully charged"}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    For reference only (doesn't affect time calculation)
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
              <p className="text-sm text-yellow-800">
                ...
                to 63.2% of the supply voltage. After 5τ, the capacitor is considered fully charged at 99.3%. 
                This calculator uses the formula t = -RC × ln(1 - percentage) for precise calculations.
              </p>
            </div>
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-semibold text-gray-900">
                            {formatNumber(formatTime(entry.result.chargeTime).value, 4)} {formatTime(entry.result.chargeTime).unit}
                          </span>
                        ...
                        <div className="text-sm text-gray-600">
                          ...
                          Target = {entry.input.targetPercentage}%
                        </div>
                      </div>
                    ))
                  )}
