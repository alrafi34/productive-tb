...
import SubnetCalculatorSEO from "./seo-content";
...
const PRESETS = [
  ...
  { label: "192.168.1.0/27", ip: "192.168.1.33", cidr: 27 },
...
export default function SubnetCalculatorUI() {
  ...
  // Sync mask input when cidr changes
  useEffect(() => {
    import("./logic").then(({ subnetMaskFromCidr, intToIp }) => {
  ...
  const run = useCallback(
    debounce((ipVal: string, cidrVal: number) => {
      ...
      setIpError(null);
      const res = calculate(ipVal, cidrVal);
      setResult(res);
    }, 120),
    []
  );
  ...
  const handleExportJson = () => {
    ...
    a.href = url;
    a.download = `subnet_${result.ip}_${result.cidr}.json`;
    a.click();
  ...
  const handleExportTxt = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `subnet_${result.ip}_${result.cidr}.txt`;
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const RESULT_ROWS = result
    ? [
        ...
        { label: "Total Addresses", value: formatNumber(result.totalAddresses), mono: false },
        { label: "Usable Hosts", value: formatNumber(result.usableHosts), mono: false },
        { label: "IP Type", value: result.ipType, mono: false },
      ]
    : [];
              ...
              {result ? (
                <>
                  ...
                  <div className="border-t border-white/20 pt-3 space-y-1 text-sm mb-4">
                    ...
                    <div className="flex justify-between">
                      <span className="text-primary-100">Broadcast</span>
                      <span className="font-mono font-semibold text-xs">{result.broadcastAddress}</span>
                    </div>
                  </div>
