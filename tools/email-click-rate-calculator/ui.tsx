...
import {
  calculateClickRate, debounce, parseNum, formatFull,
  ...
  type ClickRateInputs, type ClickRateResult, type HistoryEntry,
} from "./logic";
...
export default function EmailClickRateCalculatorUI() {
  const [inputs, setInputs] = useState<ClickRateInputs>(DEFAULT_INPUTS);
  const [result, setResult] = useState<ClickRateResult>(calculateClickRate(DEFAULT_INPUTS));
  const [copied, setCopied] = useState(false);
  const [shareCopied, setShareCopied] = useState(false);
  const [showHistory, setShowHistory] = useState(false);
  const [history, setHistory] = useState<HistoryEntry[]>([]);
  const [snapA, setSnapA] = useState<{ inputs: ClickRateInputs; result: ClickRateResult } | null>(null);
  const [snapB, setSnapB] = useState<{ inputs: ClickRateInputs; result: ClickRateResult } | null>(null);
  ...
  const run = useCallback(
    debounce((inp: ClickRateInputs) => { setResult(calculateClickRate(inp)); }, 150),
    []
  );

  useEffect(() => { run(inputs); }, [inputs, run]);

  const set = <K extends keyof ClickRateInputs>(field: K, val: ClickRateInputs[K]) =>
    setInputs((p) => ({ ...p, [field]: val }));

  const handlePreset = (p: typeof PRESETS[0]) => {
    setInputs((prev) => ({ ...prev, clicks: p.clicks, delivered: p.delivered }));
  ...
  const handleDownloadCSV = () => {
    ...
    a.download = `email-click-rate-${Date.now()}.csv`; a.click(); URL.revokeObjectURL(a.href);
  };

  const handleDownloadJSON = () => {
    const blob = new Blob([buildJSONReport(result, inputs)], { type: "application/json" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
    a.download = `email-click-rate-${Date.now()}.json`; a.click(); URL.revokeObjectURL(a.href);
  };
    ...
    <>
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2" style={{ fontFamily: "var(--font-heading)" }}>Email Click Rate (CTR) Calculator</p>
          <p className="text-xs text-gray-400 font-mono">Click Rate (%) = (Unique Clicks ÷ Delivered Emails) × 100</p>
        ...
        <div className="grid lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 space-y-5">
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-4">
              ...
              <NumField refEl={primaryRef} id="ecrc-clicks" label="Unique Clicks" value={inputs.clicks} onChange={(v) => set("clicks", v)} placeholder="250" hint="Number of unique recipients who clicked a link in the email." />
              <NumField id="ecrc-delivered" label="Delivered Emails" value={inputs.delivered} onChange={(v) => set("delivered", v)} placeholder="5000" hint="Total number of successfully delivered emails." />

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5" htmlFor="ecrc-precision">Decimal Places</label>
                <select id="ecrc-precision" value={inputs.decimalPrecision} onChange={(e) => set("decimalPrecision", parseInt(e.target.value, 10))}
          ...
          <div className="lg:col-span-7 space-y-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              <div className="flex flex-col sm:flex-row items-center gap-5">
                <ClickRateGauge pct={result.clickRatePct} color={result.tier.color} />
                <div className="flex-1 w-full">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1" style={{ fontFamily: "var(--font-heading)" }}>Click Rate — Result</p>
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-2 mb-3">
                    <div className="h-2 rounded-full transition-all duration-500" style={{ width: `${Math.min(100, (result.clickRatePct / 15) * 100)}%`, backgroundColor: result.tier.color }} />
                  </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <MetricCard label="Unique Clicks" value={formatFull(inputs.clicks)} />
              <MetricCard label="Non-Clicks" value={formatFull(result.nonClicks)} />
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-gray-50">
                {TIERS.map((t, i) => {
                  ...
                  const range = t.max === Infinity ? `Above ${TIERS[i - 1]?.max ?? 0}%` : `Up to ${t.max}%`;
                  return (
                  ...
                  {([["A", snapA], ["B", snapB]] as const).map(([label, s]) => (
                    <div key={label} className="px-5 py-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-sm font-semibold font-mono text-gray-800">{s ? `${s.result.clickRatePct.toFixed(2)}%` : "— not set —"}</span>
                      </div>
                      {s && (
                        <div className="w-full bg-gray-100 rounded-full h-1.5">
                          <div className={`h-1.5 rounded-full transition-all duration-500 ${label === "A" ? "bg-blue-400" : "bg-primary"}`}
                            style={{ width: `${Math.min(100, (s.result.clickRatePct / (Math.max(snapA?.result.clickRatePct ?? 0, snapB?.result.clickRatePct ?? 0) || 1)) * 100)}%` }} />
                        </div>
                      )}
                    </div>
                  ))}
                ...
                <div className="divide-y divide-gray-50 max-h-72 overflow-y-auto">
                  {history.length === 0 ? (
                    ...
                    <div key={entry.id} onClick={() => { setInputs(entry.inputs); setShowHistory(false); }} className="p-4 hover:bg-gray-50 cursor-pointer transition-colors">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-semibold text-gray-900">{entry.result.clickRatePct.toFixed(2)}% click rate</span>
