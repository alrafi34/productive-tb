...
import { RealPowerInputs, RealPowerResult } from "./types";
import {
  calculateRealPower,
  validateInputs,
  saveToHistory,
...
export default function RealPowerCalculatorUI() {
  ...
  const [result, setResult] = useState<RealPowerResult | null>(null);
  ...
  useEffect(() => {
    debouncedCalculate();
  }, [voltage, current, powerFactor, debouncedCalculate]);

  const calculate = () => {
    setError(null);
    
    const inputs: RealPowerInputs = {
    ...
    try {
      const calcResult = calculateRealPower(inputs);
  ...
  const handleApplyPreset = (preset: typeof presets[0]) => {
    ...
    setPowerFactor(preset.powerFactor.toString());
  };

  const handleCopy = () => {
    if (result) {
      const text = `Real Power: ${formatNumber(result.realPower, 2)} W`;
  ...
  const handleExportText = () => {
    if (result) {
      ...
      downloadFile(text, 'real_power_calculation.txt');
    }
  };
  ...
  const loadFromHistory = (entry: any) => {
    ...
    setPowerFactor(entry.input.powerFactor.toString());
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-blue-900 mb-1">Real Power Calculator</h3>
              <p className="text-sm text-blue-800">
                Calculate real (active) power in AC circuits using voltage, current, and power factor. Get instant results with step-by-step explanation.
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="bg-primary rounded-xl border border-primary-light shadow-lg shadow-primary/20 p-6 text-white space-y-4">
                ...
                <div className="mt-4 pt-4 border-t border-white/20 text-sm space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{formatNumber(result.reactivePower, 2)} VAR</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-primary-100">Power Factor:</span>
                    <span className="font-semibold">{result.powerFactor}</span>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  step="0.01"
                />
                <p className="text-xs text-gray-500 mt-1">Value between 0 and 1 (1 = pure resistive load)</p>
              </div>
            ...
            {result && !error && (
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                ...
                <div className={`p-4 rounded-lg border-2 ${
                  ...
                  result.powerFactor >= 0.85 ? 'bg-blue-50 border-blue-200' :
                  result.powerFactor >= 0.7 ? 'bg-yellow-50 border-yellow-200' :
                  'bg-red-50 border-red-200'
                }`}>
                  <p className={`text-sm font-medium ${
                    result.powerFactor >= 0.95 ? 'text-green-800' :
                    result.powerFactor >= 0.85 ? 'text-blue-800' :
                    result.powerFactor >= 0.7 ? 'text-yellow-800' :
                    'text-red-800'
                  }`}>
                    {result.efficiency}
                  </p>
                ...
                {presets.map((preset, index) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-primary font-semibold mt-1">
                      {preset.voltage}V, {preset.current}A, PF={preset.powerFactor}
                    </div>
                  </button>
                ))}
              </div>
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                    ...
                    <div className="text-xs text-blue-700">Volt-Amperes (VA)</div>
                  </div>
                  <div className="p-3 bg-purple-50 rounded-lg border border-purple-200">
                    ...
                    <div className="text-xs text-purple-700">Volt-Amperes Reactive (VAR)</div>
                  </div>
                </div>
              </div>
            )}
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-semibold text-gray-900">
                            P: {formatNumber(entry.result.realPower, 2)} W
                          </span>
                        ...
                        <div className="text-sm text-gray-600">
                          V: {entry.input.voltage}V • I: {entry.input.current}A • PF: {entry.input.powerFactor}
                        </div>
                      </div>
                    ))
                  )}
