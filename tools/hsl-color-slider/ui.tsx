...
import { HSLColor, ColorPalette, PaletteType } from './types';
import { 
  getColorFormats, 
  generateRandomColor, 
  generatePalette, 
...
export default function HSLColorSlider() {
  ...
  const ColorPreview = ({ hslColor, size = 'large' }: { hslColor: HSLColor; size?: 'small' | 'large' }) => (
    <div 
      className={`rounded-xl border-2 border-gray-200 ${size === 'large' ? 'h-28 sm:h-32' : 'h-14 w-14 sm:h-16 sm:w-16'}`}
    ...
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        ...
        <input
          ...
          className="w-14 sm:w-16 px-2 py-1 text-sm border border-gray-300 rounded"
          min="0"
          max={max}
        />
      </div>
      <div className="relative">
        <input
          ...
          value={value}
          onChange={(e) => onChange(Number(e.target.value))}
          className="w-full h-5 sm:h-6 rounded-lg appearance-none cursor-pointer slider"
  ...
  return (
    <>
      <div className="max-w-6xl mx-auto px-4 py-4 sm:px-6 sm:py-6 space-y-6 sm:space-y-8">
        {/* Header */}
        <div className="text-center space-y-4">
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900">HSL Color Slider</h1>
        ...
        <div className="grid lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
          ...
          <div className="space-y-4 sm:space-y-6 min-w-0">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
              <h3 className="text-base sm:text-lg font-semibold mb-4">Color Preview</h3>
              <ColorPreview hslColor={color} />
              <div className="mt-4 text-center">
                <span className="inline-block max-w-full break-all text-sm sm:text-lg font-mono bg-gray-100 px-3 py-1 rounded">
                  {formatHSL(color)}
                </span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 space-y-6">
              <h3 className="text-base sm:text-lg font-semibold">HSL Controls</h3>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-primary text-white px-4 py-2 rounded-lg hover:bg-primary/90 transition-colors text-sm sm:text-base"
              >
          ...
          <div className="space-y-4 sm:space-y-6 min-w-0">
            {/* Color Formats */}
            <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
              <h3 className="text-base sm:text-lg font-semibold mb-4">Color Formats</h3>
              <div className="space-y-3">
                {Object.entries(formats).map(([format, value]) => (
                  <div key={format} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 bg-gray-50 p-3 rounded-lg">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-primary hover:text-primary/80 transition-colors text-sm self-start sm:self-auto flex-shrink-0"
                    >
            ...
            {/* UI Preview */}
            <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
              <h3 className="text-base sm:text-lg font-semibold mb-4">UI Preview</h3>
              <div className="space-y-4">
                <button 
                  className="w-full sm:w-auto px-4 py-2 rounded-lg text-white font-medium"
            ...
            {history.length > 0 && (
              <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
                <h3 className="text-base sm:text-lg font-semibold mb-4">Recent Colors</h3>
        ...
        {/* Color Palettes */}
        <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
          <h3 className="text-base sm:text-lg font-semibold mb-4">Color Palettes</h3>
          <div className="flex flex-wrap gap-2 mb-4">
            {(['analogous', 'triadic', 'tetradic', 'monochromatic', 'complementary'] as PaletteType[]).map((type) => (
              <button
                key={type}
                onClick={() => generateColorPalette(type)}
                className="px-3 py-1 text-xs sm:text-sm bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors capitalize"
              >
                {type}
              </button>
            ))}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
                {selectedPalette.colors.map((paletteColor, index) => (
                  <div key={index} className="text-center space-y-2 flex flex-col items-center min-w-0">
        ...
        <div className="text-center">
          <button
            onClick={exportData}
            className="inline-flex w-full sm:w-auto justify-center items-center gap-2 bg-gray-900 text-white px-6 py-3 rounded-lg hover:bg-gray-800 transition-colors"
          >
